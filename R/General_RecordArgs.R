...
#'
#' `RecordArgs()` is a utility function that captures and records the evaluated
#' forms of arguments passed to the parent function. It returns a tibble with
#' columns named after the arguments, containing their evaluated values only.
#'
...
RecordArgs <- function(ExportPath = NULL) {
  ...
  # # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| #
  # Validate that RecordArgs is called within a function
  # # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| #
  ...
  # # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| #
  # Handle output based on ExportPath
  # # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| #
  ...
  if (is.null(ExportPath)) {
    ...
    IASDT.R::SaveAs(
      InObj = result,
      OutObj = paste0("Args_", calling_func_clean),
      OutPath = ExportPath)
