...
#' 
#' @param String A string containing the coordinates in the format "POINT (longitude latitude)". This parameter is required and cannot be NULL.
#' @param Long_Name,Lat_Name A string specifying the name to be used for the longitude and Longitude columns in the output tibble. Defaults to "Longitude" and "Latitude".
#' @return A tibble with two columns containing the longitude and latitude values extracted from the input string. The names of these columns are determined by the `Long_Name` and `Lat_Name` parameters. If no names are provided, the default names ("Longitude" and "Latitude") are used.
#' @name Text2Coords
...
Text2Coords <- function(String = NULL, Long_Name = "Longitude", Lat_Name = "Latitude") {
