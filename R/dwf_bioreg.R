...
bioreg_process <- function(env_file = ".env") {
  ...
  path_bioreg <- path_raw <- path_interim <- bioreg_url <- path_grid <-
    rast <- path_grid_raw <- geometry <- bioreg_id <- NULL
  ...
  if (length(bioreg_url_2) != 1L) {
    ecokit::stop_ctx(
      paste0("Download link extraction failed. Found: ", length(bioreg_url_2)),
      bioreg_url_2 = bioreg_url_2, length_bioreg_URL2 = length(bioreg_url_2),
      include_backtrace = TRUE)
  }
  ecokit::cat_time(bioreg_url_2, level = 2L, cat_timestamp = FALSE)
  ...
  download_command <- stringr::str_glue(
    'curl -J --create-dirs --output-dir {path_raw} -o\\
    "{zip_file_name}" -L {bioreg_url_2} --silent --max-time 300')
  ...
  repeat {
    ...
    invisible(ecokit::system_command(download_command))
  ...
  map_levels_new <- dplyr::mutate(map_levels, bioreg_id = seq_len(dplyr::n()))
