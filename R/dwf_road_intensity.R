...
road_intensity <- function(env_file = ".env") {
  ...
  env_vars_to_read <- tibble::tribble(
    ...
    "path_road", "DP_R_roads_processed", FALSE, FALSE,
    "path_road_raw", "DP_R_roads_raw", FALSE, FALSE,
    "path_road_interim", "DP_R_roads_interim", FALSE, FALSE,
  ...
  ecokit::quietly(
    expr = {
      plots_length <- patchwork::wrap_plots(
        plots_length, ncol = 3, nrow = 2) +
  ...
  ecokit::quietly(
    expr = {
      plots_distance <- patchwork::wrap_plots(
        plots_distance, ncol = 3, nrow = 2) +
        patchwork::plot_annotation(
