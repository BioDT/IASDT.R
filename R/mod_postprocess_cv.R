...
mod_postprocess_cv_1_cpu <- function(
  ...
  cv_data_fitted <- fs::path(cv_dir, "cv_data_fitted.RData")
  if (!file.exists(cv_data_fitted)) {
    ecokit::stop_ctx(
      "cv_data_fitted file not found.", cv_data_fitted = cv_data_fitted,
      include_backtrace = TRUE)
  }
  ...
  cv_data_fitted <- ecokit::load_as(cv_data_fitted) %>%
    dplyr::mutate(
      LF = purrr::map2(
        .x = cv_name, .y = cv,
        .f = ~ {
  ...
  lf_in_files <- list.files(
    path = temp_dir, pattern = "^lf_new_sites_commands_.+.txt",
  ...
  if (length(lf_in_files) == 0) {
    ecokit::stop_ctx(
      ...
      temp_dir = temp_dir, lf_in_files = lf_in_files,
      length_lf_in_files = length(lf_in_files), include_backtrace = TRUE)
  }

  ecokit::cat_time(
    paste0("Found ", length(lf_in_files), " files"),
    level = 2L, cat_timestamp = FALSE)
  stringr::str_remove_all(lf_in_files, paste0(temp_dir, "/")) %>%
    purrr::walk(ecokit::cat_time, level = 3L, cat_timestamp = FALSE)

  # Read and merge commands from input files
  lf_commands <- purrr::map(
    lf_in_files, readr::read_lines, progress = FALSE) %>%
    unlist() %>%
    gtools::mixedsort()
  ...
  lf_slurm_script <- stringr::str_glue(
    ...
    'split_file="${{output_dir}}/tf_chunk_$(printf "%03d" ',
    '"$SLURM_ARRAY_TASK_ID").txt"\n\n',

    "# Check if the target file exists\n",
    'if [[ ! -f "$split_file" ]]; then\n',
    '    echo "Error: file $split_file not found." >&2\n',
    "    exit 1\nfi\n\n",
...
mod_postprocess_cv_2_cpu <- function(
  ...
  path_cv_data_fitted <- fs::path(path_cv, "cv_data_fitted.RData")
  if (!file.exists(path_cv_data_fitted)) {
    ecokit::stop_ctx(
      "Required file 'cv_data_fitted.RData' not found",
      path_cv_data_fitted = path_cv_data_fitted, include_backtrace = TRUE)
  }

  cv_data_fitted <- ecokit::load_as(path_cv_data_fitted)
  if (nrow(cv_data_fitted) < 1) {
    ecokit::stop_ctx(
      "cv_data_fitted data should contain at least one row",
      cv_data_fitted = cv_data_fitted, include_backtrace = TRUE)
  }

  cv_data_fitted <- cv_data_fitted %>%
    dplyr::mutate(
  ...
  ecokit::save_as(
    object = cv_data_fitted, object_name = "cv_data_fitted",
    out_path = path_cv_data_fitted)
  ...
  summary_all <- cv_data_fitted$path_preds_summary %>%
