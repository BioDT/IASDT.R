...
predict_maps <- function(
  ...
  if (tar_predictions) {
    ...
    # tar all tiff files in each prediction option folder

    tar_predictions <- function(
    ...
    if (length(l_dirs_preds) > 0L) {
      ...
      tar_predictions0 <- future.apply::future_lapply(
        X = l_dirs_preds, FUN = tar_predictions, delete_tiff = TRUE,
        overwrite_tar = TRUE, future.scheduling = Inf, future.seed = TRUE,
        future.packages = c("stringr", "fs"),
        future.globals = c("l_dirs_preds", "tar_predictions"))
      rm(tar_predictions0, l_dirs_preds, envir = environment())
