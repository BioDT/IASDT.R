...
#'   execution. Default: "`commands_to_fit`".
...
mod_slurm <- function(
    ...
    path_hmsc = NULL, command_prefix = "commands_to_fit",
  ...
  project_num <- path_gpu_check <- NULL
  ...
  env_vars_to_read <- tibble::tribble(
    ...
    "project_num", "DP_R_lumi_gpu", FALSE, FALSE,
  ...
  command_list <- list.files(
    model_dir, pattern = command_prefix, full.names = TRUE)
  n_command_files <- length(command_list)
  if (n_command_files == 0) {
    ecokit::stop_ctx(
      "The file containing the bash commands does not exist",
      n_command_files = n_command_files, command_list = basename(command_list),
      include_backtrace = TRUE)
  }

  if (is.null(slurm_path_out)) {
    # This folder was created in the mod_prepare_hpc function
    slurm_path_out <- fs::path(model_dir, "model_fitting_hpc", "jobs_log")
  }

  purrr::walk(
    .x = seq_len(n_command_files),
    .f = function(x) {

      slurm_suffix <- basename(command_list[x]) %>%
        stringr::str_remove_all(
          paste(c(command_prefix, "cv_", ".txt"), collapse = "|")) %>%
        stringr::str_remove_all("^_")

      if (nzchar(slurm_suffix)) {
        out_file <- c(slurm_prefix, slurm_suffix) %>%
          paste(collapse = "_") %>%
          paste0(".slurm")
        job_name_0 <- stringr::str_remove_all(out_file, "_fit|_bash|.slurm$")
      } else {
        if (n_command_files == 1) { # nolint: unnecessary_nesting_linter
          out_file <- paste0(slurm_prefix, ".slurm")
          job_name_0 <- job_name
        } else {
          out_file <- paste0(slurm_prefix, "_", x, ".slurm")
          job_name_0 <- paste0(job_name, "_", x)
        }
      }

      n_jobs <- R.utils::countLines(command_list[x])[1]
      ...
      f <- file(fs::path(model_dir, out_file), open = "wb")
      ...
      cat2("# -----------------------------------------------------------")
      cat2(paste0("#SBATCH --job-name=", job_name_0))
      ...
      cat2(paste0("#SBATCH --account=", project_num))
      ...
      cat2(paste0("#SBATCH --array=1-", n_jobs, "\n"))
      ...
      cat2("# -----------------------------------------------------------")
      cat2(paste0("file=", command_list[x], "\n"))
      ...
      cat(paste0("\t sbatch ", fs::path(model_dir, out_file), "\n"))
    })
