...
A tibble containing detailed information about various climate variables
in `CHELSA` climate data. Each row in the tibble represents a different
climate variable, with columns providing additional details such as the
long name, unit, scale, offset, and an explanation of the variable
...
#> Warning: data set ‘chelsa_variables’ not found
...
Difference between the Maximum Temperature of Warmest month and the
Minimum Temperature of Coldest month (bio5-bio6)
...
The Coefficient of Variation = 100\*standard deviation of the monthly
precipitation / mean
...
heat sum of all days above the 0 deg C temperature accumulated over 1
year.
...
heat sum of all days above the 5 deg C temperature accumulated over 1
year.
...
heat sum of all days above the 10 deg C temperature accumulated over 1
year.
...
precipitation sum accumulated on all days during the growing season
based on TREELIM
...
Number of days with snowcover calculated using the snowpack model
